...
describe 'manila::logging' do
  ...
  shared_examples 'basic default logging settings' do
    it 'configures manila logging settings with default values' do
      is_expected.to contain_oslo__log('manila_config').with(
  ...
  shared_examples_for 'logging params unset' do
   [ :logging_context_format_string, :logging_default_format_string,
     ...
     :log_date_format, ].each { |param|
        it { is_expected.to contain_oslo__log('manila_config').with("#{param}" => '<SERVICE DEFAULT>') }
      }
  end
  ...
  on_supported_os({
    ...
    context "on #{os}" do
      ...
      it_behaves_like 'manila-logging'
